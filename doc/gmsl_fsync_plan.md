...
- [√] 4.2 长跑 ≥30min —— **通过**。四路各抓满 54000 帧(30 min),全部 30.00 fps,无掉帧无报错;
      全程每分钟采样 `FSYNC_0=0x14` 恒定、`FSYNC_22` bit6 `FSYNC_LOCKED` 恒 1 / bit7 `LOSS_OF_LOCK` 恒 0、
      `FSYNC_ERR_CNT` 除启动瞬间的 `0x09`/`0x03` 外恒 `0x00`。停流后驱动正确关回 `0x0C`
