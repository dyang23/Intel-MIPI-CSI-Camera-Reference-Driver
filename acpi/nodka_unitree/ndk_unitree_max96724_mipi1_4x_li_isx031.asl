 ...
 * deserializer, whose CSI-2 output is wired to the SoC MIPI1 port.
 *
 * Camera topology:
 *
 *                      MAX96724 (DES0, @0x27, I2C1)
 *                      ┌────────────────────────────┐
 * Link 0 (port A) ─────┤ CH00 → SER0 (MAX9295A @0x62)├──┐
 *    └─ CAM0 ISX031     │  └─ CAM0 (i2c alias 0x54) │  │
 * Link 1 (port B) ─────┤ CH01 → SER1 (MAX9295A @0x62)│  │
 *    └─ CAM1 ISX031     │  └─ CAM1 (i2c alias 0x55) │  ├─→ PHY0 (cphy A)
 * Link 2 (port C) ─────┤ CH02 → SER2 (MAX9295A @0x62)│  │        │
 *    └─ CAM2 ISX031     │  └─ CAM2 (i2c alias 0x56) │  │        ↓
 * Link 3 (port D) ─────┤ CH03 → SER3 (MAX9295A @0x62)│  │   MIPI port 1
 *    └─ CAM3 ISX031     │  └─ CAM3 (i2c alias 0x57) │  │   (IPU75XA)
 *                      └────────────────────────────┘──┘
 *
 * Board / hardware configuration:
 *   - IPU MIPI input port             : MIPI port 1   -> DES_TO_MIPI_PORT = 1
 *       ^ THIS is what differs from the other unitree files (they use mipi0).
 *         The 4x LI ISX031 module here is connected to the board's mipi1
 *         GMSL connector.
 ...
 *   - 4x LI ISX031 (Leopard Imaging), one per GMSL Link 0..3 (port A..D),
 *     each fronted by its own MAX9295A serializer (remote I2C 0x62), 4 data
 *     lanes camera-side. See acpi/max96724_li_isx031.asl for the single-link
 *     LI reference this file fans out to four links.
 *
 ...
 *       is fixed by board routing. On this board PHY0 = "cphy A" was the only
 *       PHY verified to produce frames on mipi0 (README PHY sweep D->C->B->A);
 *       we reuse PHY0 here. If mipi1 shows I2C/GMSL-lock OK but no video, sweep
 *       the internal PHY (DES_INTERNAL_PHY 4->5->6->7) the same way.
 *
...
DefinitionBlock ("", "SSDT", 2, "", "IMG_IPU", 0x20260513)
{
    ...
    Scope (\_SB.PC00)
    {
        Device (DES0)
        {
            ...
            // ---- DES-level configuration (NODKA unitree board, mipi1 connector) ----
            ...
            // ---- LI ISX031 #1 on GMSL input Link 1 (port B) ----
            ...
            // ---- LI ISX031 #3 on GMSL input Link 3 (port D) ----
